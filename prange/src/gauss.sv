...
localparam int popcount = 7; // read bit, add
localparam int stall = 8;
...
reg [31:0] mat_write_buffer;
reg [31:0] mat_read_buffer;
...
reg [$clog2(HEIGHT)-1:0] eliminate_head; // write pointer
...
reg [31:0] word_row_cache; // cache of the current row being used for elimination
...
always_ff @(posedge clk or negedge reset) begin
    if (!reset) begin
        ...
        case (state)
            ...
            receive : begin
                mat_write_buffer[(unaligned_w_ptr+1)%32] <= mat_bit;
            ...
            select : begin
                ...
                if (r_row_ptr == HEIGHT+1) begin // a cycle after the one read two cycles ago was the last valid row 
                    ...
                    if (mat_read_buffer[scan_col_ptr%32]) begin // value from two cycles ago
            ...
            eliminate : begin
                ...
                if (eliminate_head-2 > eliminate_tail) begin
                    eliminate_head <= 0; // begin set up for next round
                end
                if (eliminate_head == 0) begin
                    eliminate_head <= 1;
                    mat_we <= 0;
                    r_row_ptr <= l_elim_row_ptr;
                    ...
                    state <= eliminate_warmup_wait;
                    if (elim_col_ptr/WORD_SIZE == (WIDTH_AUG-1)/WORD_SIZE) begin
